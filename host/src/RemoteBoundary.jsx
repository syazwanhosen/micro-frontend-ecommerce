...
function FailedRemote({ name }) {
  const url = `http://localhost:${REMOTES[name]?.port}`;
  return (
    <div className="remote-error">
      <span style={{ fontSize: 34 }} aria-hidden="true">🔌</span>
      <h3 style={{ color: 'var(--text)' }}>The “{name}” micro frontend is unavailable</h3>
      <p className="small">
        The rest of the page still works — that is the point of the split. Start it with{' '}
        <code>npm run serve -w remote-{name}</code> so <code>{url}</code> responds.
      </p>
